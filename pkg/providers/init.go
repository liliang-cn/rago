...
func InitializeEmbedder(ctx context.Context, cfg *config.Config, factory *Factory) (domain.Embedder, error) {
	...
	if cfg.Providers.ProviderConfigs.Ollama != nil || cfg.Providers.ProviderConfigs.OpenAI != nil || cfg.Providers.ProviderConfigs.LMStudio != nil {
		...
		providerConfig, err := GetEmbedderProviderConfig(&cfg.Providers.ProviderConfigs, cfg.Providers.DefaultEmbedder)
		if err != nil {
...
func InitializeLLM(ctx context.Context, cfg *config.Config, factory *Factory) (domain.Generator, error) {
	...
	if cfg.Providers.ProviderConfigs.Ollama != nil || cfg.Providers.ProviderConfigs.OpenAI != nil || cfg.Providers.ProviderConfigs.LMStudio != nil {
		...
		// Use single provider
		providerConfig, err := GetLLMProviderConfig(&cfg.Providers.ProviderConfigs, cfg.Providers.DefaultLLM)
		if err != nil {
...
// convertLLMPoolConfig converts config.LLMPoolConfig to LLMPoolConfig
func convertLLMPoolConfig(cfg *config.LLMPoolConfig) (LLMPoolConfig, error) {
	poolConfig := LLMPoolConfig{
	...
	case "round_robin":
		poolConfig.Strategy = RoundRobinStrategy
	case "random":
		poolConfig.Strategy = RandomStrategy
	case "least_load":
		poolConfig.Strategy = LeastLoadStrategy
	case "failover":
		poolConfig.Strategy = FailoverStrategy
	default:
		poolConfig.Strategy = RoundRobinStrategy // Default
	}
