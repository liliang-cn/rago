...
func isBuiltInStandaloneAgentName(name string) bool {
	...
	case strings.ToLower(defaultAssistantAgentName), strings.ToLower(defaultStakeholderAgentName):
		return true
	default:
		return false
	}
...
func (m *SquadManager) ensureBuiltInStandaloneAgent(ctx context.Context, builtin *AgentModel) error {
	...
	if err == nil {
		...
		if len(existing.MCPTools) == 0 {
			existing.MCPTools = append([]string(nil), builtin.MCPTools...)
		}
		existing.EnableRAG = true
		existing.EnableMemory = true
		existing.EnableMCP = true
