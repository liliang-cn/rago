...
dev:
	...
	@$(GOCMD) run main.go serve --verbose

## ingest: Ingest sample documents
ingest:
	@echo "Ingesting sample documents..."
	@$(GOCMD) run main.go ingest docs/ --recursive --verbose

## query: Interactive query mode
query:
	@echo "Starting interactive query mode..."
	@$(GOCMD) run main.go query --interactive

## reset: Reset the database
reset:
	@echo "Resetting database..."
	@$(GOCMD) run main.go reset --force

## list: List documents
list:
	@echo "Listing documents..."
	@$(GOCMD) run main.go list
...
profile:
	...
	@$(GOCMD) run main.go serve --cpuprofile=cpu.prof --memprofile=mem.prof
...
release: clean
	...
	# Linux AMD64
	@GOOS=linux GOARCH=amd64 $(GOBUILD) $(LDFLAGS) -o $(BUILD_DIR)/releases/$(BINARY_NAME)-linux-amd64 ./main.go
	
	# Linux ARM64
	@GOOS=linux GOARCH=arm64 $(GOBUILD) $(LDFLAGS) -o $(BUILD_DIR)/releases/$(BINARY_NAME)-linux-arm64 ./main.go
	
	# macOS AMD64
	@GOOS=darwin GOARCH=amd64 $(GOBUILD) $(LDFLAGS) -o $(BUILD_DIR)/releases/$(BINARY_NAME)-darwin-amd64 ./main.go
	
	# macOS ARM64
	@GOOS=darwin GOARCH=arm64 $(GOBUILD) $(LDFLAGS) -o $(BUILD_DIR)/releases/$(BINARY_NAME)-darwin-arm64 ./main.go
	
	# Windows AMD64
	@GOOS=windows GOARCH=amd64 $(GOBUILD) $(LDFLAGS) -o $(BUILD_DIR)/releases/$(BINARY_NAME)-windows-amd64.exe ./main.go
