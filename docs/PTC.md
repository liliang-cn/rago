...
PTC 是 RAGO 的创新功能，允许 LLM 生成 JavaScript 代码来编排工具调用，而非传统的 JSON 参数方式。
...
### 4. RAGORouter (`pkg/ptc/router.go`)

工具路由器，将 `callTool()` 调用路由到实际服务：

```go
func (r *RAGORouter) Route(ctx context.Context, toolName string, args map[string]interface{}) {
    // 1. 尝试已注册的处理器
